...
abstract class AbstractGameObject extends Sprite implements Animatable {
	...
	bool intersects(AbstractGameObject otherGameObject) {
		return collisionEnabled && otherGameObject.collisionEnabled &&
			this.hitTestObject(otherGameObject);//this.boundsTransformed.intersects(otherGameObject.boundsTransformed);
	}
	
	List<Segment> getSegments() {
		Vector topLeft = new Vector(boundsTransformed.left, boundsTransformed.top);
		Vector topRight = new Vector(boundsTransformed.right, boundsTransformed.top);
		Vector bottomLeft = new Vector(boundsTransformed.left, boundsTransformed.bottom);
		Vector bottomRight = new Vector(boundsTransformed.right, boundsTransformed.bottom);
