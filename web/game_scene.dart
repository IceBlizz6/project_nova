...
class GameScene implements Animatable {
	...
	bool advanceTime(num time) {
		...
		List<IntersectionData> polygons = CollisionRay.getSightPolygon(segments, playerObject.position);
	...
	void setupPlayerObject(Vector startingPosition) {
		...
		ControllableGameObject gameObj = new ControllableGameObject(this, _gameLoop.keyboardDevice, _gameLoop.mouseDevice, _gameLoop.gamepadDevice);
	...
	void addShipVisibility() {
		...
		VisibilityGameObject gameObj = new VisibilityGameObject(this, shape, bitmapData, playerObject);
	...
	Vector collisionCheck2(AbstractGameObject gameObj, Vector startPosition, Vector targetPosition) {
		...
		for (int i = 1;i<=lerpSteps;i++) {
			Vector testPosition = startPosition.lerp(targetPosition, i.toDouble() / lerpSteps);
	...
	Vector checkCollisionMovement(AbstractGameObject gameObj, Vector startPosition, Vector targetPosition) {
